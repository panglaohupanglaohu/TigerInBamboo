# -*- coding: utf-8 -*-
"""生成 TigerMessenger 子项目清单 .docx（嵌入无头 WebGL 渲染截图）"""
...
IMG_DIR = r"D:\TigerInBamnoo\tools\shot\images"
...
def add_image_cell(cell, img_name, width=Cm(2.2)):
    cell.text = ''
    p = cell.paragraphs[0]
...
def make_table(headers, rows, col_widths=None, image_map=None, image_key_idx=0):
    # 若提供 image_map，自动追加“预览”列
    has_img = bool(image_map)
    display_headers = list(headers) + (['预览'] if has_img else [])
    t = doc.add_table(rows=1, cols=len(display_headers))
    t.style = 'Light Grid Accent 1'
    t.alignment = WD_TABLE_ALIGNMENT.CENTER
    hdr = t.rows[0].cells
    for i, htext in enumerate(display_headers):
    ...
    for row in rows:
        ...
        if has_img:
            key = row[image_key_idx]
            add_image_cell(cells[len(row)], image_map.get(key))
    if col_widths:
        cw = list(col_widths) + ([2.4] if has_img else [])
        for row in t.rows:
...
add_para('说明：“菜单可放置”指该资产已登记进 src/core/buildingCatalog.js 的 BUILDING_CATALOG，'
         ...
         '“预览”列为使用无头 Chromium + SwiftShader 对工厂渲染后的 512×512 截图，方便用户直接对照资产外观。',
...
make_table(
    ...
    col_widths=[2.5, 3, 5, 5.8],
    image_map=SCENE_IMAGES,
    image_key_idx=0
)
add_para('')
add_para('独立入口场景（未注册进场景系统）', bold=True)
ENTRY_IMAGES = {
    'planet.html': 'planet.png',
    'aircraft.html': None,
    'retro_spaceship.html': None,
    'retro_spaceship_amber.html': None,
    'moebius_aircraft_outline.html': None,
}
make_table(
    ['入口', '名称', '说明'],
    [
    ...
    col_widths=[4.5, 4, 7.8],
    image_map=ENTRY_IMAGES,
    image_key_idx=0
)
add_para('')
add_para('信使主岛内地标分区（messengerIsland.js 的 landmarks）', bold=True)
LANDMARK_IMAGES = {
    '起始营地': 'startingCamp.png',
    '月牙湖': 'moonLake.png',
    '背侧大湖': 'moonLake.png',
    '修船厂码头': 'oldHarbor.png',
    '莫比斯水晶城': 'moebiusCity.png',
    '莫比斯湖沼': 'swampZone.png',
    '厚涂苔丘': 'mossyGround.png',
    '云环 / 赤道风暴云墙 / 大峡谷': 'equatorialClouds.png',
}
make_table(
    ['分区', '说明'],
    [
...
ARTIFACT_IMAGES = {
    ...
    '焦墨岩 / 黑岩': 'rock.png',
    '小渔船': 'fisherBoat.png',
    '港口起重机': 'harborCrane.png',
    '货柜木箱堆': 'stackedCrates.png',
    '修船厂码头': 'oldHarbor.png',
    '莫比斯湖沼（地形器物）': 'moebiusSwamp.png',
    '莫比斯塔（母皇塔 / 金鳞沿轨塔）': 'moebiusTower.png',
    '莫比斯航空艇（垂绳登艇·可驾驶）': 'moebiusAirship.png',
    '莫比斯飞船 / 飞行器编队（可进驾驶舱）': None,
    '基督城电车（可乘坐）': 'tram.png',
    '气泡座舱（绕花厅巡游）': 'bubblePod.png',
    '木栅栏': 'fence.png',
    '桥': 'bridge.png',
    '低多边形树': 'lowPolyTree.png',
    '云朵': 'cloud.png',
    '水晶巨构群（InstancedMesh）': 'moebiusCity.png',
    '金黄能量海': 'moebiusCity.png',
    '庭园石组（立/侍/卧/桥/瀑/座六式）': 'saihoji.png',
    '石阶 / 踏石': 'platforms.png',
    '原住民人偶': 'swamp_nativeDoll.png',
    '贝壳': 'swamp_shell.png',
    '莲叶舟': 'swamp_lotusLeafBoat.png',
    '池水 / 洪隐山石壁 / 叠水瀑布 / 苔岩岛': 'moonLake.png',
    '太空水环 / 崖壁叠瀑 / 山洞': 'startingCamp.png',
}
make_table(
    ['器物', '工厂来源', '菜单可放置'],
    [
...
CREATURE_IMAGES = {
    '阿狸·小狐狸（可跟随 / 对话）': 'fox.png',
    '赛博水墨虎（巡游 + 下坑饮水）': 'moebiusTiger.png',
    'Boids 小鸟群（峡谷高空）': 'bird.png',
    '花厅楼顶鸟群（母皇塔尖环绕）': 'moebiusCity.png',
    '花厅巡航鸟 / 送别伴飞编队': 'moebiusCity.png',
    '异星滑翔长翼鸟（航空艇护航队）': 'longWingGlider.png',
    '丹顶鹤 NPC / 岩上鹤': 'craneNPC.png',
    '弹琴老人（山洞旁 NPC）': 'startingCamp.png',
    '送信人（玩家角色）': 'messenger.png',
    '数字孪生送信人': 'agentMessenger.png',
    '任务 NPC：小虎': 'messenger.png',
    '任务 NPC：阿竹': 'messenger.png',
    '任务 NPC：月见': 'messenger.png',
    '任务 NPC：星野': 'messenger.png',
    '任务 NPC：驿站': 'messenger.png',
    '任务 NPC：远方': 'messenger.png',
    '任务 NPC：月影': 'messenger.png',
    'planet NPC：红方 / 绿方 / 蓝方': 'planet.png',
    '沼泽·白鲸（昂首破水）': 'swamp_whale.png',
    '沼泽·黄绿鳗形生物': 'swamp_eel.png',
    '沼泽·橙红管状蠕虫丛': 'swamp_tubeWorm.png',
    '沼泽·粉色长尾垂挂生物（蛞蝓/水母感）': 'swamp_pinkHanger.png',
    '沼泽·沼泽鸟': 'swamp_bird.png',
    '沼泽·长尾猴（投果互动）': 'swamp_monkey.png',
    '沼泽·发光蜥蜴': 'swamp_lizard.png',
    '沼泽·发光带鱼': 'swamp_ribbonFish.png',
    '沼泽·绿黑斑纹小鱼群': None,
    '沼泽·萤火虫': None,
}
make_table(
...
PLANT_IMAGES = {
    ...
    '草坪山丘': 'lawnHill.png',
    '草丛': 'grassTuft.png',
    '书店绣球（变体）': 'bookshopHydrangeas.png',
    '红枫': None,
    '竹林（竹竿 + 介字撇叶簇）': None,
    '厚涂苔藓地被 / 苔丘': 'mossyGround.png',
    '苔藓斑块 / 石底苔裙': 'mossyGround.png',
    '沼泽·世界树': 'swamp_worldTree.png',
    '沼泽·苍天巨树': 'swamp_toweringTree.png',
    '沼泽·坑缘棕榈 / 丛林树': 'swamp_rimPalm.png',
    '沼泽·树冠顶棚（巨叶遮天）': 'swamp_canopy.png',
    '沼泽·紫色蘑菇 / 珊瑚状植物': 'swamp_mushroom.png',
    '沼泽·发光花蕊花': 'swamp_glowFlower.png',
    '沼泽·巨花': 'swamp_giantFlower.png',
    '沼泽·粉色花蕊尖锥': 'swamp_stamenSpike.png',
}
make_table(
    ['植物', '工厂来源', '菜单可放置'],
    [
...
CATALOG_IMAGES = {
    ...
    'pole': 'pole.png',
    'rock': 'rock.png',
    'fisherBoat': 'fisherBoat.png',
    'harborCrane': 'harborCrane.png',
    'stackedCrates': 'stackedCrates.png',
    'oldHarbor': 'oldHarbor.png',
    'pine': 'pine.png',
    'hydrangea': 'hydrangea.png',
    'flower': 'flower.png',
    'lawnHill': 'lawnHill.png',
    'fox': 'fox.png',
    'moebiusSwamp': 'moebiusSwamp.png',
}
...
make_table(
    ...
    col_widths=[3, 4.5, 5.5, 2],
    image_map=CATALOG_IMAGES,
    image_key_idx=0
)
add_para('')
...
out = r'D:\TigerInBamnoo\docs\TigerMessenger清单_含截图.docx'
doc.save(out)
print('SAVED:', out)
