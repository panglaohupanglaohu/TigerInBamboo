...
export class CustomAgent {
  constructor(scene, record, grove, config) {
    this.record = record;
    this._grove = grove;
    ...
    // 家在溪涧西侧雪原（干燥、离虎巡游线不远不近，便于互动）
    this.home = new THREE.Vector3(-5, 0, 8);
    ...
    if (record.anatomyType === "AVES") {
      this._bird = new BirdAgent(scene, {
        ...
        forage: [this.home.x, this.home.z],
        perch: [-8.5, 3.5],
      });
      this.group = this._bird.group;
      // 记录身高与基准站高（0.42m）的等比缩放
  ...
  /** 外观件：眼睛贴头骨（按体宽比例缩放）；SALTATORIAL 补长耳 */
  _buildDetails() {
  ...
  /** 漫游目标：优先家周 8m 内竹竿，其次涧边干燥点 */
  _pickTarget() {
    const R = 8;
    const cands = (this._grove?.bamboos ?? []).filter(
      (b) => Math.hypot(b.x - this.home.x, b.z - this.home.z) < R
    );
    if (cands.length) {
      const b = cands[(Math.random() * cands.length) | 0];
      return new THREE.Vector3(b.x + (Math.random() - 0.5) * 1.4, 0, b.z + (Math.random() - 0.5) * 1.4);
    }
    for (let i = 0; i < 8; i++) {
  ...
  update(dt, time, tiger) {
    ...
    // —— 关系矩阵：与虎的互动 ——
    const rel = (this.record.relations ?? []).find((r) => r.target === "tiger");
    const td = tiger ? gp.distanceTo(tiger.group.position) : Infinity;
    const strength = rel?.strength ?? 0.5;
    this._fleeing = Math.max(0, this._fleeing - dt);
    if (rel?.type === "predator-prey" && td < 4 + strength * 6) {
      // 恐惧惊逃：背向虎全力窜
      const away = new THREE.Vector3().subVectors(gp, tiger.group.position).setY(0).normalize();
      ...
      // 互利亲近：缓步向虎靠拢
      this._target = tiger.group.position.clone();
      this.state = "WALK";
      this.stateLabel = "亲近";
      this._timer = 2;
    ...
    if (this.saltatorial) {
      ...
      gp.y = groundHeight(gp.x, gp.z) + push * 0.09 * moving;
    } else {
      // 趾行/蹄行：匀速推进
