...
## 西芳寺起始庭园：垂直层叠与水石穿插（2026-08-02 14:30 起，主人命题）

> 负责人标注：**Kimi** 实现并验收。对象：`world/startGarden.js`（起始视角园景）。

### 四项重构

1. **洪隐山地基**：`createMountainWall()` 返回 `mountainGroup`——基座 4 块 Icosahedron 错落
   咬合 + 中层 2 岩 1 方嵌缝 + 顶部 3 块 BoxGeometry 叠出山脊（最高 ~5.6 ≈ 玩家 3.3 倍），
   立于画面右侧（-5.4, 9.6）
2. **石缝生树 / 青苔裹石**：红叶移入山脚岩缝（`rotateZ(0.24)` 斜插 + 根部下沉 0.12），
   `addMossRing()` 在山脚底缘与树根各撒一圈极扁翠绿多边形，模糊石地交界
3. **竹林背景墙**：`createBambooWallCluster()` 12 竿一簇、高 6.2~8.4（玩家 3.6~4.9 倍）、
   双排错位密植，两簇放到 z≈16.2/16.8 画面最深处
4. **叠水**：`createCascade()` 五层阶梯——台面水唇（横扁块）+ 跌落面（竖扁块）沿山壁
   朝池面逐级下探，浅蓝 0x9fd8e8 微发光，末端泡沫入池；动画兼容原 `streams/foam` 驱动

### 迭代记录

- 第一轮叠水嵌进山腹不可见；第二轮错判贴面方向（组局部 +z 朝北背池）；
  第三轮改挂朝池（+x 东）面，薄轴换向，截图确认阶梯跌水可读

### 验收（2026-08-02 14:49）

- 三件套通过（控制台零告警）
- 放大截图核验：石壁高耸、红叶出缝、跌水成阶、竹墙深远、池水在前

### Todos

- [x] 洪隐山 mountainGroup（Kimi，14:35）
- [x] 叠水阶梯（Kimi，14:36 → 14:47 三轮修位）
- [x] 石缝生树 + 苔环（Kimi，14:40）
- [x] 竹林背景墙（Kimi，14:38）
- [x] 苔环/叠水节奏按实机微调（**Grok**，2026-08-02）
  - 叠水 6 层、层距收紧、泡沫更密；苔环双圈 + 色阶加丰；跌水动画放缓
- [x] 六景其他景区跟进同一套垂直层叠语言（**Grok**，2026-08-02）
  - 各景区石底苔裙双圈；主石/枯瀑脚嵌小石；枯瀑补浅蓝阶梯唇
