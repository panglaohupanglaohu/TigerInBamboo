...
func run()->void:
    ...
    var material=StandardMaterial3D.new();material.shading_mode=BaseMaterial3D.SHADING_MODE_UNSHADED;material.albedo_color=Color(1,0,1);material.cull_mode=BaseMaterial3D.CULL_DISABLED
    for mesh in w.model.find_children("citadel-coastal-cliff-seal","MeshInstance3D",true,false):
        if mesh.is_visible_in_tree():mesh.material_override=material
    var out="res://../artifacts/pipeline/citadel-west-massif/"
    for night in [false,true]:
        w.window_lights.set_enabled(night)
        await process_frame
        await RenderingServer.frame_post_draw
        root.get_texture().get_image().save_png(out+("godot-seal-identify-night.png" if night else "godot-seal-identify-day.png"))
