...
func run()->void:
    ...
    w.camera.global_position=frame*Vector3(-38,12,61)
    w.camera.look_at(frame*Vector3(-49,-1,35),frame.basis.y.normalized())
    w.camera.fov=38
    ...
    var out="res://../artifacts/pipeline/citadel-foundation-support/"
