...
func run()->void:
    ...
    var out="res://../artifacts/pipeline/citadel-tier-lighting/"
    for night in [false,true]:
        w.window_lights.set_enabled(night)
        await process_frame
        await RenderingServer.frame_post_draw
        root.get_texture().get_image().save_png(out+("godot-before-" if OS.get_cmdline_user_args().has("--legacy-city-lighting") else "godot-after-")+("night.png" if night else "day.png"))
