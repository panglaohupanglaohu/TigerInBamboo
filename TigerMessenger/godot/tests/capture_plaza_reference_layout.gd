...
func run()->void:
    ...
    w.camera.global_position=frame*Vector3(10,25,116)
    w.camera.look_at(frame*Vector3(7,5,73),frame.basis.y.normalized())
    w.camera.fov=45
    ...
    var out="res://../artifacts/pipeline/citadel-plaza-reference-layout/"
