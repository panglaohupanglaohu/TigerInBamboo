...
func run()->void:
    var expected=JSON.parse_string(FileAccess.get_file_as_string("res://../artifacts/pipeline/citadel-front-coastal-slope/report.json"))
    ...
    if meshes.size()==1 and meshes[0].is_visible_in_tree():
        ...
        for row in expected.reports[1].edgeVertices:
            total+=1
    ...
    var passed:bool=total>10 and matched==total
    var report={"tolerance_m":.01,"max_matched_error":max_error,"used_perimeter_vertices":total,"matched_current_web":matched,"missing":missing,"passed":passed,"scope":"Actual visible Godot foreground coastal top vertex positions match corrected Web mesh; local frame, not a second water model."}
    var f=FileAccess.open("res://../artifacts/pipeline/citadel-front-coastal-slope/godot-check.json",FileAccess.WRITE);f.store_string(JSON.stringify(report,"  "));f.close()
