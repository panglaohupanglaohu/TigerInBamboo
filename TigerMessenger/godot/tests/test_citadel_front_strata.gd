...
func run()->void:
    var expected=JSON.parse_string(FileAccess.get_file_as_string("res://../artifacts/pipeline/citadel-front-strata/check.json"))
    ...
    if meshes.size()==1 and meshes[0].is_visible_in_tree():
        ...
        for row in expected.meshes[0].edgeVertices:
            total+=1
    ...
    var passed:bool=total>100 and matched==total
    var report={"tolerance_m":.01,"max_matched_error":max_error,"used_perimeter_vertices":total,"matched_current_web":matched,"missing":missing,"passed":passed,"scope":"Actual visible Godot Blender foreground strata vertex positions match corrected Web mesh; local frame, not a second water model."}
    var f=FileAccess.open("res://../artifacts/pipeline/citadel-front-strata/godot-check.json",FileAccess.WRITE);f.store_string(JSON.stringify(report,"  "));f.close()
