...
func run()->void:
    ...
    candidate.code=FileAccess.get_file_as_string("res://shaders/citadel_terrain_color.gdshader").replace("render_mode cull_disabled;", "render_mode cull_disabled, unshaded;")
    ...
    for night in [false,true]:
        ...
        root.get_texture().get_image().save_png(out+("godot-unlit-night.png" if night else "godot-unlit-day.png"))
