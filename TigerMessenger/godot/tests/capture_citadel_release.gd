...
func run()->void:
    ...
    await process_frame
    var patch=w.castle_adapter.original.find_child("CitadelReleasedLayout",true,false)
    assert(patch!=null,"Normal citadel scene did not load released layout")
    assert(w.window_lights.points.filter(func(light):return light.shadow_enabled).size()==2)
    var plaza=true
    ...
    assert(w.window_lights.points[1].position.distance_to(Vector3(65,9,80))<0.001)
    ...
    for night in [false,true]:
        ...
        var evidence=w.window_lights.evidence()
        assert(evidence.matching==evidence.total)
        var evidence_file=FileAccess.open(out+"released-lighting-"+("night" if night else "day")+".json",FileAccess.WRITE)
