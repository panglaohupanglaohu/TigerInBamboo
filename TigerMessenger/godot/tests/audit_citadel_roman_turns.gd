...
func run()->void:
    ...
    for role in ["gladius","spear","longbow"]:
        ...
        for point in c.route:
            var world:Vector3=c.root.to_global(point)
            if route.is_empty() or route[-1].distance_to(world)>0.001:route.append(world)
        for i in range(1,route.size()-1):
            var incoming:Vector3=(route[i]-route[i-1]).slide(up).normalized()
            var outgoing:Vector3=(route[i+1]-route[i]).slide(up).normalized()
            var start:=Basis(incoming,up,incoming.cross(up)).orthonormalized().get_rotation_quaternion()
            var finish:=Basis(outgoing,up,outgoing.cross(up)).orthonormalized().get_rotation_quaternion()
            var subdivisions:=maxi(1,int(ceil(start.angle_to(finish)/deg_to_rad(2))))
            var foot:Vector3=route[i]
            var hit=space.intersect_ray(PhysicsRayQueryParameters3D.create(foot+up*0.16,foot-up*0.2))
            if hit.is_empty():failures.append({"point":i,"reason":"unsupported"});continue
            for k in range(subdivisions+1):
                samples+=1
                var query=PhysicsShapeQueryParameters3D.new();query.shape=shape
                query.transform=Transform3D(Basis(start.slerp(finish,float(k)/subdivisions)),Vector3(hit.position)+up*(-lowest+0.015+0.14))
                if not space.intersect_shape(query,1).is_empty():
                    ...
                    else:failures.append({"point":i,"angle_fraction":float(k)/subdivisions,"reason":"turn_mesh_blocked","parts":blocked})
    ...
    var report={"roles":results,"scope":"Actual carried meshes at each waypoint with orientation interpolated in at most 2 degree increments; fixed 0.14 step-up reserve. Discrete rotational sampling, not a mathematical continuous sweep or walking animation.","passed":results.values().all(func(r):return r.passed)}
    FileAccess.open("res://../artifacts/pipeline/citadel-entry/roman-turns.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "))
