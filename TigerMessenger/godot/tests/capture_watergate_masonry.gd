...
func run()->void:
    ...
    w.camera.global_position=frame*Vector3(-35,7,98)
    w.camera.look_at(frame*Vector3(-25,1,77),frame.basis.y.normalized())
    w.camera.fov=43
    ...
    var out="res://../artifacts/pipeline/citadel-watergate-masonry/"
