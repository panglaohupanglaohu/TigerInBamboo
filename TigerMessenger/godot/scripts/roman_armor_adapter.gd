class_name RomanArmorAdapter
extends RefCounted
## Reversible body-local wearable contract: artifacts/pipeline/roman-armor-v2/assembly.json.
...
func set_enabled(value: bool) -> bool:
    ...
    for row in _crests:
        var adjusted: Transform3D = row.transform
