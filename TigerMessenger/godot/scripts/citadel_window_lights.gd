...
func bind_environment(world:Node)->void:
    ...
    if profile is Dictionary:
        night_environment=profile
        var p=profile.position;var t=profile.target
        night_direction=world.model.global_basis*(Vector3(p[0],p[1],p[2])-Vector3(t[0],t[1],t[2])).normalized()
    ...
    for sun in world.find_children("*","DirectionalLight3D",true,false):
        sun_rows.append({"sun":sun,"energy":sun.light_energy,"color":sun.light_color,"basis":sun.global_basis})
func set_enabled(value:bool)->void:
    enabled=value
    for row in environment_rows:
        row.env.background_color=Color("223950") if value else row.background
        row.env.ambient_light_color=Color(night_environment.get("ambientColor","8baee6")) if value else row.ambient
        row.env.ambient_light_energy=float(night_environment.get("ambientEnergy",0.5)) if value else row.energy
    for row in sun_rows:
        row.sun.light_energy=float(night_environment.get("energy",0.2)) if value else row.energy
        row.sun.light_color=Color(night_environment.get("color","84a5d6")) if value else row.color
        if value and night_direction.length_squared()>0.5:
            row.sun.global_basis=Basis.looking_at(-night_direction,Vector3.UP)
        elif not value:row.sun.global_basis=row.basis
