...
const ARMOR_PATH := "res://assets/art-pilots/roman-armor-v2.glb"
const ASSEMBLY_RELATIVE := "../artifacts/pipeline/roman-armor-v2/assembly.json"
...
func _test_variant(side: String) -> void:
	...
	var fixture := _attach_armor(nodes)
	if fixture.is_empty():
...
func _attach_armor(nodes: Dictionary) -> Dictionary:
	var packed := load(ARMOR_PATH) as PackedScene
	_check(packed != null, "armor GLB loads")
	if packed == null:
		return {}
	...
	for id in assembly["hideOriginalNodeIds"]:
		_hide_with_rest(nodes[str(id)], visibility)
	var crest_rest: Dictionary = {}
	var offset: Array = assembly["crestOffset"]
	for id in assembly["crestNodeIds"]:
		var crest: Node3D = nodes[str(id)]
		crest_rest[crest] = crest.transform
		crest.position += Vector3(offset[0], offset[1], offset[2])
	return {"instance":instance, "visibility":visibility, "crestRest":crest_rest}


func _hide_with_rest(node: Node, visibility: Dictionary) -> void:
	if node is Node3D:
		visibility[node] = node.visible
		node.hide()
	if assembly.get("hideDescendants", false):
		for child in node.get_children():
			_hide_with_rest(child, visibility)


func _restore_armor(fixture: Dictionary) -> void:
	fixture["instance"].free()
	for node in fixture["visibility"]:
		node.visible = fixture["visibility"][node]
		_check(node.visible == fixture["visibility"][node], "armor fixture restores original visibility")
	for node in fixture["crestRest"]:
		node.transform = fixture["crestRest"][node]
		_check(node.transform == fixture["crestRest"][node], "armor fixture restores original crest transform")
