...
export function createWarshipClearance(boat,obstacleGroups) {
  ...
  function intersects(node,t,box){if(!node.box.intersectsBox(box))return null;if(node.items)return node.items.find(item=>item.box.intersectsBox(box)&&crossing(item.t,t))||null;return intersects(node.left,t,box)||intersects(node.right,t,box);}
