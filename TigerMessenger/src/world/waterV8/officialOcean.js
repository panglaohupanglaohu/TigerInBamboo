...
/** Drop geodesic ocean vertices into the crystal-city canyon; keep sea level elsewhere. */
export function drapeOceanOnLegacyPlanet(positions, radius, seaLevel = OFFICIAL_OCEAN_SEA_LEVEL) {
  ...
  for (let i = 0; i < positions.length; i += 3) {
    ...
    _dir.normalize();
    const canyon = canyonOffsetDir(_dir);
    const level = canyon < -0.45 ? canyon + 0.28 : seaLevel;
