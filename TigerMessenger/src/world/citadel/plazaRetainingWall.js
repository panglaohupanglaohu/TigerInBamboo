...
import baked from '../../../assets/models/optimized/citadel-plaza-retaining/plazaRetainingR01.js';
...
export function buildPlazaRetainingWall(castle,radius=160){
 ...
 const piers=[];
 for(const x of [46,53.5,63,72,80]){
 ...
 const bays=[];for(let j=0;j<23;j++){
  const x=45.2+(j+.5)*(35.42/23),half=35.42/46;
 ...
 for(const top of [3.65,1.8]){const b=new THREE.Mesh(new THREE.BoxGeometry(35.5,.16,.74),trim);b.name='plaza-retaining-stringcourse';b.position.set(62.91,top,88.3);b.castShadow=true;b.receiveShadow=true;b.userData.citadelSolidExterior=true;root.add(b);}
