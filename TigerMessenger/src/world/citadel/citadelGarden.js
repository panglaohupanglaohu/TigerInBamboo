...
export function buildCitadelGarden(){
 ...
 const materials=data.materials.map(m=>{const p=m.pbrMetallicRoughness;return new THREE.MeshStandardMaterial({color:new THREE.Color().setRGB(...p.baseColorFactor.slice(0,3)),roughness:.93});});
 const parts=data.parts.map(p=>{const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(p.position,3));g.setAttribute('normal',new THREE.Float32BufferAttribute(p.normal,3));g.setIndex(p.index);return {part:p,geometry:g};});
 const trees=[[48,4,80,.86],[72,4,80,.98],[46.3,4,58,.78],[73.7,4,58,.88],[44.5,10,33,.92],[75.5,10,33,1],[40,16,12,1.12],[80,16,12,.96]];
 trees.forEach(([x,y,z,size],i)=>{const tree=new THREE.Group();tree.position.set(x,y,z);tree.scale.setScalar(size);tree.rotation.y=i*.91;
  for(const {part:p,geometry} of parts){const mesh=new THREE.Mesh(geometry,materials[p.material]);mesh.position.fromArray(p.translation);mesh.quaternion.fromArray(p.rotation);mesh.scale.fromArray(p.scale);mesh.castShadow=true;mesh.receiveShadow=true;tree.add(mesh);}root.add(tree);
 });
 const shrubs={shrubs:[]};
