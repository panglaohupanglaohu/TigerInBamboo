...
- [x] 54. 阿狸全场景整容重构（Kimi 2026-08-03，主人命题）
  - `assets/aliFox.js` `createClassicAliFox()`：SphereGeometry 圆球大头（r1.5/8×6）
    嵌套卵圆身（身体略小于头，深度嵌套消灭方块断层）；尖耳+乳白耳内
  - 火焰尾：4 节极扁 Cone 串联（后臀起、整体 rotation.x=0.6 上翘 35°、逐节回卷）
    前 3 节动漫橙 #E96A36、尾尖乳白 #F4F7ED 修长；总长 ≈ 身体 1.1 倍
  - 短小四足（5 段圆柱+焦黑爪尖）；眯眯眼（焦黑弧形）/乳白倒三角粗眉/粉腮红
  - `createAliFoxCompanion()`：尾随玩家 ~2.2 单位、贴地行走；四腿对角正弦交替
    + 身体轻颠 + 尾巴微风飘逸；全件 Cel + addOutline
  - 验收：背影/耳/尾尖截图 ✓ 跟随移动 ✓ 三件套通过
