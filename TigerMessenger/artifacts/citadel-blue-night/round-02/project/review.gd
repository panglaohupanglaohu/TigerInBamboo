...
func capture() -> void:
    ...
    for enabled in [false,true]:
        for lamp in street_lights: lamp.visible=enabled
        for i in range(12): await get_tree().process_frame
        await RenderingServer.frame_post_draw
        var file="../street-on.png" if enabled else "../street-off.png"
        get_viewport().get_texture().get_image().save_png(ProjectSettings.globalize_path("res://"+file))
        report.views.append({"street_lights_enabled":enabled,"file":file,"draw_calls":RenderingServer.get_rendering_info(RenderingServer.RENDERING_INFO_TOTAL_DRAW_CALLS_IN_FRAME),"primitives":RenderingServer.get_rendering_info(RenderingServer.RENDERING_INFO_TOTAL_PRIMITIVES_IN_FRAME),"local_lights":street_lights.size() if enabled else 0,"shadow_lights":3 if enabled else 1})
    report["scope"]="Round 2 isolated local light-pool study; original WFC geometry unchanged, no street navigation or reflection acceptance."
