...
static func install(root:Node3D)->Dictionary:
    ...
    var gardens=[]
    for row in [[8,9],[-4,14],[-16,20]]:
        for side in [-1,1]:
            var p=Vector3(side*23,float(row[1]),float(row[0]))
            ...
            for j in range(3):
                var variant="cypress" if j==1 else "broadleaf"
                var t=load("res://assets/"+variant+"-reference-v1.glb").instantiate()
                root.add_child(t)
                t.position=p+Vector3((j-1)*1.1,.25,(j%2)*1.5-1)
                t.scale=Vector3.ONE*(1.1 if j==1 else 1.3)
            gardens.append(p)
    ...
    for w in root.find_children("*","MeshInstance3D",true,false):
        ...
        for i in range(0,f.size(),3):
            var hit=Geometry3D.segment_intersects_triangle(Vector3(3,40,37),Vector3(3,-40,37),w.global_transform*f[i],w.global_transform*f[i+1],w.global_transform*f[i+2])
