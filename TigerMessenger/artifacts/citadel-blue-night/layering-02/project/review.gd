...
func set_blue(value: bool) -> void:
    ...
    environment.ambient_light_energy=0.28 if blue else 0.42
    ...
    sun.light_energy=0.5 if blue else 0.7
    label.text="圣城层次第二轮 · 1总览 / 2水岸 / 3侧面 / L街灯 / 空格日夜" if blue else "原工厂日景 · Godot 同镜头 | 空格切换"
...
func capture() -> void:
    ...
    label.visible=false
    report.views=[]
    for mode in ["batched","shore-reflection","side","waterfront","boat"]:
        ...
        camera.fov=48
        if mode=="boat":
            camera.position=Vector3(12,7,46)
            camera.look_at(Vector3(3,0,33))
        elif mode=="waterfront":
            camera.fov=54
            camera.position=Vector3(16,26,82)
            camera.look_at(Vector3(0,16,6))
        elif mode.begins_with("shore"):
            camera.position=Vector3(10,10,53)
            camera.look_at(Vector3(0,12,8))
        elif mode=="side":
            camera.position=Vector3(-40,26,58)
    ...
    report["scope"]="Layering iteration 2: isolated four-level city candidate; full gameplay/navigation not implemented."
