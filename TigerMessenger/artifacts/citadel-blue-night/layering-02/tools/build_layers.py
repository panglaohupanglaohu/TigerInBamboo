...
for o in list(bpy.data.objects):
 ...
 if 'highland-central-sacred-tower' in anc:
  ...
  continue
 # Cut at common depth planes, then move every material surface with the same slab.
 # This replaces independent component offsets, which separated adjoining materials.
 bm=bmesh.new();bm.from_mesh(o.data)
 for zcut in [12,0,-12]:
  bmesh.ops.bisect_plane(bm,geom=list(bm.verts)+list(bm.edges)+list(bm.faces),dist=.00001,plane_co=inv@Vector((0,-zcut,0)),plane_no=m.to_3x3().transposed()@Vector((0,1,0)),clear_inner=False,clear_outer=False)
 bm.faces.ensure_lookup_table()
 for f in bm.faces:
  f.select=True
 # Split every face before translation; all vertices of each face receive one offset.
 bmesh.ops.split_edges(bm,edges=list(bm.edges))
 moved=0;groups={}
 for f in bm.faces:
  mid=m@f.calc_center_median();dy=rise(-mid.y);groups[f.index]=True
  if dy:
   for v in f.verts:w=m@v.co;w.z+=dy;v.co=inv@w
   moved+=1
...
(r/'layer-report.json').write_text(json.dumps({'method':'Common depth-plane cuts with rigid face-slab translation across material surfaces; central tower compressed vertically to 72 percent and raised 9m','bands':[0,4,9,15],'changes':changes,'navigation_validated':False},indent=2))
