...
code=code.replace('d.x*1.035,d.y*1.035,d.z*.57','d.x*1.14,d.y*1.14,d.z*1.42')
...
base_rings=[]
for z,radius in [(ground,.46),(.10,.39),(.33,.29),(.66,.235)]:
 ring=[]
 for k in range(18):
  angle=k*math.tau/18
...
report={'seed':811,'stage':'single Blender v2 hero candidate, actual GLB reimport rendered; NOT Web/Godot integrated or batch complete',
 ...
 'geometryChanges':{'clumpCenterInputsRetained':len(scope['clumps']),'originalBranchSegmentsRetained':len(scope['segments']),'verticalClumpFactorFromOriginal':1.42,'v1VerticalFactor':.57,'horizontalClumpFactor':1.14,'woodBudget':3400,'foliageBudget':5600,'flaredRootUnion':True,'barkReliefMaxLocal':0},
