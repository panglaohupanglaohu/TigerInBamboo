...
bpy.ops.wm.open_mainfile(filepath=str(s.SOURCE));scene=bpy.context.scene;obs={s.ident(o):o for o in scene.objects if s.ident(o)};records=[];ps=json.loads((s.ART/'source-poses.json').read_text());parts=[obs['n'+str(n+5*i)] for i in range(26) for n in [64,66]]
for frame in range(1,302):
 scene.frame_set(frame)
 for key,values in ps['frames'][min(frame-1,180)]['transforms'].items():
  if ':i' not in key:s.setm(obs[key],s.mat(values))
 bpy.context.view_layer.update();g=geo(parts);hits=crossings(g,g);hits=[h for h in hits if (int(h[0][1:])-64)//5 != (int(h[1][1:])-64)//5]
 if hits:records.append({'frame':frame,'pairs':hits})
r={'passed':not records,'frames':301,'scope':'Different original oars: shaft and blade triangle-face intersections over every saved frame. Same-oar connections excluded.','failures':records};(s.ART/'source-oar-neighbor-validation.json').write_text(json.dumps(r,indent=2));print(json.dumps({'passed':not records,'failedFrames':len(records)}))
