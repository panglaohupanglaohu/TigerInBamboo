...
for frame in range(1,302):
 ...
 record={'frame':frame,'handleBody':crossings(geo(handles),geo(body)),'oarDeck':crossings(geo(handles+shafts),deckgeo),'legsDeck':crossings(geo(legs),deckgeo)}
 if record['handleBody'] or record['oarDeck'] or record['legsDeck']:records.append(record)
 if frame<=121:
  for i in range(26):
