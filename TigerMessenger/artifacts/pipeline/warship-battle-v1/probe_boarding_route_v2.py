...
for j,(x,z,yaw) in enumerate(route):
 ...
 for key in ['n24','n27']:
  ...
  for vertex in foot.data.vertices:
   p=foot.matrix_world@vertex.co;hit=support[3].ray_cast(Vector((p.x,p.y,4)),Vector((0,0,-1)),5)
   if hit[0] is not None:lift=max(lift,hit[0].z+.001-p.z)
