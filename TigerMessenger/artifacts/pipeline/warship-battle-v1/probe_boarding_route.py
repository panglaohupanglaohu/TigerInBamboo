...
for j,(x,z,yaw) in enumerate(route):
 ...
 s.setm(control,Matrix.Translation((x,floor-.0094164694,z))@Matrix.Rotation(yaw,4,'Y')@Matrix.Diagonal((1/1.7,1/1.7,1/1.7,1)));bpy.context.view_layer.update();hits=crossings(bg,geo(walker));records.append({'sample':j,'x':x,'y':floor-.0094164694,'z':z,'yaw':yaw,'contacts':hits})
(s.ART/'boarding-route-probe.json').write_text(json.dumps({'carryTransforms':{roman.ident(o):roman.flat(roman.local(o)) for o in new if roman.ident(o)},'selfContacts':selfhits,'samples':records},indent=2));print(json.dumps({'samples':len(records),'clear':sum(not r['contacts'] for r in records),'failed':[{k:v for k,v in r.items() if k!='y'} for r in records if r['contacts']],'selfContacts':selfhits},indent=2))
