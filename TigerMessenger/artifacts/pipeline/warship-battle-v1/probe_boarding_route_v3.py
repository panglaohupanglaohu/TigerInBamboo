...
def crossings(a,b):
 ...
 return sorted(hits)
bpy.ops.wm.open_mainfile(filepath=str(s.OUT/'warship-battle-v1.blend'));scene=bpy.context.scene;scene.frame_set(241);bpy.context.view_layer.update();rail=next(o for o in scene.objects if s.ident(o)=='n253');rail.data=rail.data.copy()
for vertex in rail.data.vertices:
 p=s.CI.to_3x3()@vertex.co;p.x=min(p.x,1.70-s.local(rail).translation.x);vertex.co=s.C.to_3x3()@p
fore=next(o for o in scene.objects if s.ident(o)=='n325');top=Vector((.55,3.77,0));bottom=Vector((2.18,.64,-.40));d=bottom-top;base=s.local(fore);old=Vector((1.63,-3.13,0));q=old.normalized().rotation_difference(d.normalized());base.translation=(0,0,0);s.setm(fore,Matrix.Translation((top+bottom)/2)@q.to_matrix().to_4x4()@base@Matrix.Diagonal((1,d.length/old.length,1,1)));bpy.context.view_layer.update();boatobs=[o for o in scene.objects if o.type=='MESH' and not o.hide_render and s.ident(o)];bg=geo(boatobs);support=geo([o for o in boatobs if s.ident(o) in ['n231','add:bow-landing'] or s.ident(o).startswith('add:deck-plank-') or s.ident(o).startswith('add:boarding-plank-')]);before=set(scene.objects);bpy.ops.import_scene.gltf(filepath=str(ROOT/'assets/models/optimized/roman-family-v1/romanSoldier_gladius_blue.glb'));new=set(scene.objects)-before;roots=[o for o in new if not o.parent];control=bpy.data.objects.new('REVIEW boarder control',None);scene.collection.objects.link(control)
