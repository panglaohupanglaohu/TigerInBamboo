...
left=Vector((.10,.26,.025));shieldrot=Matrix.Identity(4)
roman.setm(nodes['n32'],Matrix.Translation(left-Vector((-.055,0,0)))@shieldrot)
roman.arm(nodes,'L',(0,.305,.067),left,(0,1,0),skin,hint=(.1,-1,-.2))
right=Vector((.045,.23,-.062));axis=Vector((.08,.996,0)).normalized()
roman.setm(nodes['n50'],Matrix.Translation(right-axis*.04)@Vector((0,1,0)).rotation_difference(axis).to_matrix().to_4x4());roman.arm(nodes,'R',(0,.305,-.067),right,axis,skin,hint=(-.2,-1,-.2))
for xi in range(15):
 x=.20+xi*.05
 for z in [-.14,-.18,-.22,-.26]:
  for yaw in [-1.57,-.8,0,.8,1.57]:
