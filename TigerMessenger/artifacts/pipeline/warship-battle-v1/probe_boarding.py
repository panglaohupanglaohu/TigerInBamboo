...
for z in [-.12,-.05,.05,.12]:
 for j in range(31):
  x=-1.25+j*2.85/30;s.setm(control,Matrix.Translation((x,.663-.0094164694,z))@Matrix.Diagonal((1/1.7,1/1.7,1/1.7,1)));bpy.context.view_layer.update();hits=crossings(bg,geo(walker));records.append({'x':x,'z':z,'contacts':hits})
summary={str(z):{'samples':31,'failed':sum(bool(r['contacts']) for r in records if r['z']==z),'pairs':sorted(set(tuple(p) for r in records if r['z']==z for p in r['contacts']))} for z in [-.12,-.05,.05,.12]};(s.ART/'boarding-probe.json').write_text(json.dumps({'summary':summary,'samples':records},indent=2));print(json.dumps(summary,indent=2))
