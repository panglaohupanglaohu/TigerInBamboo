...
for name,frame,offset in [('v2-diagnostic-front-fill',31,(4,-.02,.45)),('v2-diagnostic-three-quarter-fill',31,(3,-4,1.2))]:
 if name=='v2-diagnostic-front-fill':
  light=bpy.data.objects.new('Diagnostic mouth fill only',bpy.data.lights.new('Diagnostic mouth fill only','AREA'));sc.collection.objects.link(light);light.location=original['n0'].matrix_world@(C.to_3x3()@Vector((38,-13,0)));aim=original['n0'].matrix_world@(C.to_3x3()@Vector((18,-12,0)));light.rotation_euler=(aim-light.location).to_track_quat('-Z','Y').to_euler();light.data.energy=1500;light.data.size=6
