...
export function buildFrontHarborApproach(castle,radius){
 ...
 const rise=(4-dockY)/3,n=Math.ceil(rise/.18),run=11,stairWidth=4,turnWidth=4.8;
 ...
 for(let f=0;f<3;f++){
  ...
  for(let i=0;i<n;i++){
   ...
   box(walking,'front-harbor-step',x,top,z,run/n+.015,top-bottom,stairWidth,paving);

   route.push([x,top,z]);
  }
  // Continuous sloping parapets leave both turn entries open.
  for(const side of [-1,1]){
   const a=from+(to-from)*.045,b=from+(to-from)*.955,ya=low+rise*.045,yb=low+rise*.955;
   ...
   const g=new THREE.ExtrudeGeometry(shape,{depth:.28,bevelEnabled:false,steps:1});
   const m=new THREE.Mesh(g,stone);m.name='front-harbor-slope-parapet';m.position.z=z+side*(stairWidth/2+.2)-.14;m.castShadow=true;m.receiveShadow=true;walls.add(m);
  }
  route.push([to,high,z]);
  if(f<2){const turnX=to+(f===0?1.5:-1.5),bottom=Math.min(dockY-1.5,sea(turnX,z-2.5)-1.5);box(walking,'front-harbor-turn',turnX,high,z-2.5,turnWidth,high-bottom,9,paving);route.push([turnX,high,z],[turnX,high,z-5],[to,high,z-5]);if(f===0)box(walls,'front-harbor-turn-guard',turnX+turnWidth/2+.15,high+1,z-2.5,.3,1,9,stone);else {box(walls,'front-harbor-turn-guard',turnX-turnWidth/2-.15,high+1,80.95,.3,1,4.7,stone);box(walls,'front-harbor-turn-guard',turnX-turnWidth/2-.15,high+1,86.05,.3,1,.7,stone);}}
 }
 box(walking,'front-harbor-plaza-link',45.5,4,80,5.2,.35,stairWidth,paving);route.push([48,4,80],[51,4,80]);
 ...
 city.userData.frontHarborApproach={dockY,flights:3,stepsPerFlight:n,riser:rise/n,tread:run/n,stairWidth,turnWidth,cuts,status:'walking approach; fleet still uses rear harbor'};
 root.userData.sourceId='citadel-front-harbor-v2';root.userData.frontHarborApproach=city.userData.frontHarborApproach;
 return root;
}
