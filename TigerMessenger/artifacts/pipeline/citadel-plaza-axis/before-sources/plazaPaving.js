...
export function buildPlazaPaving(){
  ...
  const circle=Array.from({length:96},(_,i)=>[Math.cos(i*Math.PI/48)*7.44,Math.sin(i*Math.PI/48)*7.44]);
  const cuts=[circle,rect(10,-10.5,22,3.5),rect(7.6,-.2,10,2.2),rect(13.2,-16,18.8,-10.5)];
  for(let row=0,z=-16.5;z<12.5;row++,z+=1.15){
    for(let col=0,x=-14.15-(row%2)*.575;x<22;col++,x+=1.15){
      const x0=Math.max(-14.15,x)+.022,x1=Math.min(22,x+1.15)-.022,z0=z+.022,z1=Math.min(12.5,z+1.15)-.022;
  ...
  Object.assign(m.userData,{sourceId:'citadel-plaza-complete-paving-v3',fieldStones,stoneCount:372+fieldStones,rings:6,outerRadius:7.4,decorativeOnly:true,skipColliders:true});
  return m;
}
