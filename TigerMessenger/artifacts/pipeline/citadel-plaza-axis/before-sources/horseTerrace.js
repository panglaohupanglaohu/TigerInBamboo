...
export const HORSE_TERRACE={x:75,z:72.5,y:5.8,width:12,depth:14,rampEnd:60,rampStart:65.5,rampWidth:5.6};
export function horseTerraceHeight(x,z){
  const t=HORSE_TERRACE;
  if(x>=69&&x<=81&&z>=65.5&&z<=79.5)return t.y;
...
export function buildHorseTerrace(stone,paving){
 const root=new THREE.Group();root.name='citadel-original-horse-terrace';
 ...
 for(let i=0;i<12;i++){const top=4+(i+1)*.15;block('horse-terrace-side-step-'+i,66.6+(i+.5)*.2,(top+3.8)/2,77,.2,top-3.8,2.4,true);}
