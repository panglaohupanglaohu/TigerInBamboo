...
for iteration in [3,4]:
 ...
 for v,original in zip(terrain.data.vertices,base):
  ...
  sideFace=(-6<z<58 and halfWidth-.5<abs(x-8)<halfWidth+5 and h<tierY-.7)
  if sideFace and not(-13<x<-7 and z>45):
   weight=min(1,max(0,(tierY-.7-h)/3))*min(1,max(0,(halfWidth+5-abs(x-8))/2))
   side=1 if x>8 else -1
...
data={'changes':changes,'source':'assets/models/optimized/citadel-cliff/citadel-cliff-r04.blend','iterations':reports}
