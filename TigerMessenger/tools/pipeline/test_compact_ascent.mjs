...
const out=new URL('../../artifacts/pipeline/citadel-compact-ascent/',import.meta.url);
...
try{for(const version of ['before','after']){
 ...
 await p.goto('http://localhost:8931/TigerMessenger/?autostart=1'+(version==='before'?'&citadelCompact=0':''),{timeout:180000});
 ...
 const data=await p.evaluate(async()=>{
  ...
  const ascent=c.userData.compactAscent,gate=c.getObjectByName('citadel-new-main-gate');
  return {foundations:c.userData.compactFoundations,ascent,mountain:c.userData.compactMountain,gatePosition:gate.position.toArray(),profile,footTests,layout:c.userData.plazaLayout,statue:c.worldToLocal(statue.getWorldPosition(new T.Vector3())).toArray(),horse:c.worldToLocal(horse.getWorldPosition(new T.Vector3())).toArray(),support:c.getObjectByName('citadel-plaza-retaining-wall').userData.support,paving:mesh.userData,triangles:a.position.count/3,downward,walkRoute:c.userData.walkRoute,horseRoute:c.userData.horsePlazaExit.map(p=>c.worldToLocal(t.scene.getObjectByName('castleContainer').localToWorld(new T.Vector3(...p))).toArray()),pictures};
 });
...
assert.equal(report.after.downward,0);assert.ok(report.after.paving.fieldStones>500);assert.ok(report.after.walkRoute.length<report.before.walkRoute.length);assert.ok(report.after.footTests.every(p=>p.supported),JSON.stringify(report.after.footTests.filter(p=>!p.supported)));assert.equal(report.after.foundations.length,4);assert.equal(report.after.layout.statueX,65);assert.equal(report.after.layout.horseX,81);assert.ok(report.after.support.bays.some(b=>b.easternReturn));
