...
data=json.loads((ROOT/'artifacts/pipeline/citadel-watergate-masonry/blender-source.json').read_text())
scene=bpy.data.scenes.get('TigerMessenger_Watergate_Review') or bpy.data.scenes.new('TigerMessenger_Watergate_Review');bpy.context.window.scene=scene
...
for i,p in enumerate(data['parts']):
 ...
 obj=bpy.data.objects.new('watergate-part-'+str(i),mesh);scene.collection.objects.link(obj)
...
folder=ROOT/'assets/models/optimized/citadel-watergate'
bpy.data.libraries.write(str(folder/'watergate-r02.blend'),{scene},fake_user=True)
(folder/'watergateR01.js').write_text('export default '+json.dumps({'source':'assets/models/optimized/citadel-watergate/watergate-r02.blend','parts':parts},separators=(',',':'))+';\n')
