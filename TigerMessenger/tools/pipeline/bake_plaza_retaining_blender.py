...
data=json.loads((ROOT/'artifacts/pipeline/citadel-plaza-retaining/blender-source.json').read_text())
scene=bpy.data.scenes.get('TigerMessenger_Plaza_Retaining_Review') or bpy.data.scenes.new('TigerMessenger_Plaza_Retaining_Review');bpy.context.window.scene=scene
...
bpy.data.libraries.write(str(folder/'plaza-retaining-r01.blend'),{scene},fake_user=True)
(folder/'plazaRetainingR01.js').write_text('export default '+json.dumps({'source':'assets/models/optimized/citadel-plaza-retaining/plaza-retaining-r01.blend','parts':parts},separators=(',',':'))+';\n')
