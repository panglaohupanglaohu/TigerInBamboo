...
data=json.loads((ROOT/('artifacts/pipeline/citadel-plaza-axis/'+('retaining-r03-blender-source.json' if WIDE else 'retaining-blender-source.json'))).read_text())
scene=bpy.data.scenes.get('TigerMessenger_Plaza_Axis_Retaining_Review') or bpy.data.scenes.new('TigerMessenger_Plaza_Axis_Retaining_Review');bpy.context.window.scene=scene
...
for i,p in enumerate(data['parts']):
 ...
 bmesh.ops.bevel(bm,geom=edges,offset=.018,segments=1,affect='EDGES',clamp_overlap=True)
 bmesh.ops.triangulate(bm,faces=list(bm.faces));bm.to_mesh(mesh);bm.free();mesh.update()
 obj=bpy.data.objects.new('plaza-edge-part-'+str(i),mesh);scene.collection.objects.link(obj)
...
folder=ROOT/'assets/models/optimized/citadel-plaza-retaining'
blend_name='plaza-retaining-r03.blend' if WIDE else 'plaza-retaining-r02.blend'
bpy.ops.wm.save_as_mainfile(filepath=str(folder/blend_name))
(folder/('plazaRetainingR03.js' if WIDE else 'plazaRetainingR02.js')).write_text('export default '+json.dumps({'source':str((folder/blend_name).relative_to(ROOT)),'parts':parts},separators=(',',':'))+';\n')
