...
data=json.loads((ROOT/('artifacts/pipeline/citadel-plaza-axis/'+('port-r04-blender-source.json' if PORT else ('edge-r03-blender-source.json' if WIDE else 'edge-blender-source.json')))).read_text())
scene=bpy.data.scenes.get('TigerMessenger_Plaza_Axis_Edge_Review') or bpy.data.scenes.new('TigerMessenger_Plaza_Axis_Edge_Review');bpy.context.window.scene=scene
...
blend_name='plaza-edge-r03.blend' if WIDE else 'plaza-edge-r02.blend'
if PORT:
