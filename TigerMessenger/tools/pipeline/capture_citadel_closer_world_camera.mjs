...
const out=new URL('../../artifacts/pipeline/citadel-residual-shore/',import.meta.url);
await mkdir(out,{recursive:true});
const browser=await chromium.launch({channel:'chrome',headless:true,args:['--use-angle=metal']});
try{
 const results=[],current=true;
 for(const candidate of (current?[false]:[false,true])){
  ...
  const report=await page.evaluate(()=>{
   ...
   t.camera.position.copy(castle.localToWorld(new T.Vector3(-40,30,115)));
   t.camera.up.set(0,1,0).transformDirection(castle.matrixWorld);
   t.camera.lookAt(castle.localToWorld(new T.Vector3(-18,20,15)));t.camera.fov=52;t.camera.far=3000;t.camera.updateProjectionMatrix();t.camera.updateMatrixWorld(true);
  ...
  await page.waitForTimeout(1600);const canvas=await page.evaluate(()=>{const t=window.__tm;t.renderer.render(t.scene,t.camera);return t.renderer.domElement.toDataURL('image/png');});await writeFile(new URL('closer-world-camera.png',out),Buffer.from(canvas.split(',')[1],'base64'));
