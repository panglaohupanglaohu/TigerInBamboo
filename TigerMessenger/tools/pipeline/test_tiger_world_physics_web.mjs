...
const naturalFrames = Math.max(24, Math.min(180, Number(process.env.TM_NATURAL_FRAMES) || 24));
const base = process.env.TM_TEST_URL || 'http://127.0.0.1:8877/TigerMessenger/';
const output = new URL(`../../artifacts/pipeline/tiger-web-v3/world-physics-${new Date().toISOString().replace(/[:.]/g, '-')}/`, import.meta.url);
await mkdir(output, { recursive: true });
const sources = ['src/world/collision.js', 'src/world/swampGround.js', 'src/world/moebiusSwamp.js', 'src/main.js', 'src/world/moebiusTiger.js', 'src/assets/characters/moebiusTiger.js', 'src/assets/tigerAnatomy.js', 'src/player/player.js', 'src/quest/questSystem.js', 'assets/models/optimized/moebiusTigerAnatomyData.js'];
...
const report = { passed: false, scope: 'Browser-only collision tracing around original physics plus separate natural/controlled dialog phases. Natural proximity and controlled activation are separate outcomes; no natural playthrough, complete spoken exchange or rescue completion claim.', startedAt: new Date().toISOString(), url: base, sourceHashesBefore: await sourceHashes(), pageErrors: [], consoleErrors: [], failedRequests: [], interventions: ['Fresh headless browser seeds chapter 4 only to inspect the original live rescue target.', 'Browser-only module wrappers trace ground and asset collision displacement and observe exact dialog arguments and calls the original implementation once; no production file modifications.', 'Natural phase repeats the original one-time local +(2,0,2) player teleport, with normal game physics for the configured number of dialog frames.', 'Controlled phase sets the test player exactly 2 world units from the live tiger immediately before each of 6 real dialog calls. Original physics and the entire original dialog implementation continue; this phase does not prove natural proximity.'] };
let page;
...
export function updateSwampTigerDialog(deps) {
  ...
  state.rows.push(row);
  if (row.frame >= (mode === 'natural' ? ${naturalFrames} : 6)) state.phase = mode + '-done';
  return result;
}
`;
try {
  ...
  await page.route('**/src/world/collision.js', async route => {
    const response = await route.fetch();
    let body = await response.text();
    body = body.replace('export function resolveCollisions(', 'function originalResolveCollisions(').replace('export function resolveAssetColliders(', 'function originalResolveAssetColliders(');
    body += `
export function resolveCollisions(...args) {
 const state=window.__tmTigerDialogDiagnostic;
 const before=args[0].clone();
 const result=originalResolveCollisions(...args);
 if(state?.phase==='natural' && (state.physics ||= []).length<100) state.physics.push({kind:'ground',before:before.toArray(),after:args[0].toArray(),delta:before.distanceTo(args[0]),radialBefore:before.length(),radialAfter:args[0].length(),groundR:args[4].groundR,hillRadius:args[6]?.sampleRadius?.(before)});
 return result;
}
export function resolveAssetColliders(...args) {
 const state=window.__tmTigerDialogDiagnostic;
 const before=args[0].clone();
 const result=originalResolveAssetColliders(...args);
 if(state?.phase==='natural' && (state.physics ||= []).length<100) state.physics.push({kind:'asset',before:before.toArray(),after:args[0].toArray(),delta:before.distanceTo(args[0])});
 return result;
}
`;
  ...
  const phases = new Set(report.natural.rows.map(row => row.after?.phase));
  report.natural.dialogueExchangeCompleted = ['tiger', 'messenger', 'cool'].every(phase => phases.has(phase));
  if (naturalFrames > 24) {
    // A completed original exchange must retain its 14-second cooldown.
    report.controlled.expectedOutcome = 'original-cooldown-preserved';
    report.controlled.passed = report.controlled.rows.length === 6 && report.controlled.rows.every(row => row.near && row.gameStarted && !row.blocked && row.after?.phase === 'cool' && !row.after?.active && row.after?.timer > 0);
  }
  report.passed = report.natural.observedFrames === naturalFrames && report.natural.passed && (naturalFrames <= 24 || report.natural.dialogueExchangeCompleted) && report.controlled.passed && report.controlled.originalActorAndUpdatePreserved && report.controlled.playerRestored && report.controlled.rescueTargetDistanceToActor < 1e-8 && report.controlled.rescue.chapter === 4 && report.browserObserver.productionSourceMatches && report.pageErrors.length === 0;
} catch (error) {
