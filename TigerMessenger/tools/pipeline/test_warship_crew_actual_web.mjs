...
const files=['src/world/saihojiPhalanx.js','src/world/warshipCrewContinuity.js','src/assets/warshipV6.js','src/assets/warshipV6Data.js'];
...
try {
 ...
 report=await page.evaluate(()=>{
  ...
  const snap=()=>({time:t,phase:battle.root.userData.campaignStatus.phase,ships:ships().map(o=>({name:o.name,source:o.userData.warshipV6.source,visible:visible(o),seats:o.userData.warshipV6.crewStatus()})),actors:actors().map(o=>({uid:o.userData.uid,uuid:o.uuid,visible:visible(o),embarked:o.userData.embarked}))});
  ...
  check('chapter2 launches original two boats with v9',outbound.ships.length===2&&outbound.ships.every(o=>String(o.source).includes('v9')));
  ...
  return {passed:checks.every(c=>c.passed),checks,outbound,ashore,returning,scope:'Actual 8931 world and v9 factory; RAF held, original scheduled chapter2 voyage and updates. Explicit real whaleReturned callback tests return identity; intervening fight and physical boarding path are not validated.'};
 });
