...
data=json.loads((ROOT/'artifacts/pipeline/citadel-harbor-curtain/blender-source.json').read_text())
scene=bpy.data.scenes.get('TigerMessenger_Harbor_Curtain_Review') or bpy.data.scenes.new('TigerMessenger_Harbor_Curtain_Review');bpy.context.window.scene=scene
...
folder=ROOT/'assets/models/optimized/citadel-harbor-curtain'
bpy.data.libraries.write(str(folder/'harbor-curtain-r01.blend'),{scene},fake_user=True)
(folder/'harborCurtainR01.js').write_text('export default '+json.dumps({'source':'assets/models/optimized/citadel-harbor-curtain/harbor-curtain-r01.blend','parts':parts},separators=(',',':'))+';\n')
