...
data=json.loads((ROOT/'artifacts/pipeline/citadel-plaza-edge/blender-source.json').read_text())
scene=bpy.data.scenes.get('TigerMessenger_Plaza_Edge_Review') or bpy.data.scenes.new('TigerMessenger_Plaza_Edge_Review');bpy.context.window.scene=scene
...
folder=ROOT/'assets/models/optimized/citadel-plaza-edge'
bpy.data.libraries.write(str(folder/'plaza-edge-r01.blend'),{scene},fake_user=True)
(folder/'plazaEdgeR01.js').write_text('export default '+json.dumps({'source':'assets/models/optimized/citadel-plaza-edge/plaza-edge-r01.blend','parts':parts},separators=(',',':'))+';\n')
