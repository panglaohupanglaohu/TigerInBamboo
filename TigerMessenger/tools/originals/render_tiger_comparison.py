...
for name,relative in [('before','assets/models/originals/blender-r3/moebiusTiger.blend'),('after','assets/models/optimized/moebiusTiger.blend')]:
    ...
    camera.location=(5,-7,4);target=Vector((0,0,1))
    camera.rotation_euler=(target-camera.location).to_track_quat('-Z','Y').to_euler()
    camera.data.type='ORTHO';camera.data.ortho_scale=5
    scene.camera=camera;scene.render.filepath=str(out/f'moebiusTiger-{name}.png')
    bpy.ops.render.render(write_still=True)
print('TIGER_COMPARISON_RENDERED')
