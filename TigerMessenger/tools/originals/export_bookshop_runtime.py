...
folder=root/'assets/models';source=folder/'originals/bookshop.source.json';data=json.loads(source.read_text())
bpy.ops.wm.open_mainfile(filepath=str(folder/'optimized/bookshop.blend'))
objects={o['three_node_id']:o for o in bpy.data.objects if 'three_node_id' in o}
C=Matrix.Rotation(math.pi/2,4,'X').inverted()
result={'version':1,'source':'original-bookshop-blender-cleanup','geometries':{},'nodes':[]}
...
output=folder/'optimized/bookshopGeometryData.js'
output.write_text('// Generated from the verified original bookshop Blender working copy.\nexport default '+json.dumps(result,separators=(',',':'))+';\n')
(folder/'optimized/bookshop.runtime-report.json').write_text(json.dumps(result['stats'],indent=2))
print('BOOKSHOP_RUNTIME_EXPORT_OK',json.dumps(result['stats']))
