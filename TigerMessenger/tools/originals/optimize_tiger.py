"""Lossless render-surface cleanup on the imported original moebiusTiger only."""
import bpy,json
from pathlib import Path
root=Path(__file__).resolve().parents[2]
src=root/'assets/models/originals/blender-r3/moebiusTiger.blend'
out=root/'assets/models/optimized/moebiusTiger.blend'
...
for old in meshes:
    ...
    if old.color_attributes:raise RuntimeError('This cleanup is scoped to the original moebiusTiger without color layers')
    ...
    for i,loop in enumerate(new.loops):
        ...
        mesh_normal_error=max(mesh_normal_error,max(abs(normals[i][k]-new.corner_normals[i].vector[k]) for k in range(3)))
    for name,uvs in uv_layers.items():
    ...
    before+=len(coords);faces+=len(original_faces)
    if mesh_normal_error>=1e-4:
        skipped.append({"mesh":old.name,"normalError":mesh_normal_error})
        after+=len(coords)
        bpy.data.meshes.remove(new)
        continue
    max_normal_error=max(max_normal_error,mesh_normal_error)
    after+=len(verts)
    for obj in users:obj.data=new
    bpy.data.meshes.remove(old)
print("VALIDATION_ERRORS", max_position_error,max_uv_error,max_normal_error)
assert max_position_error==0 and max_uv_error==0 and max_normal_error<1e-4
report={"preservedMeshes":skipped,'asset':'original moebiusTiger','source':str(src.relative_to(root)),'result':str(out.relative_to(root)),'verticesBefore':before,'verticesAfter':after,'facesUnchanged':faces,'maxFaceCornerPositionError':max_position_error,'maxFaceCornerNormalError':max_normal_error,'maxUVError':max_uv_error,'materialIndicesUnchanged':True,'runtimeIntegrated':False,'renderParity':'Original Three shader adaptation still pending; no runtime replacement'}
bpy.context.scene['optimization_report']=json.dumps(report)
bpy.ops.wm.save_as_mainfile(filepath=str(out),compress=True)
(out.parent/'moebiusTiger.optimization.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
print('ORIGINAL_TIGER_CLEANUP_OK',json.dumps(report))
