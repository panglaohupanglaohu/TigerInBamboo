...
/** 鲸体配色（主人 2026-09-05 参考水彩座头鲸）：普鲁士深蓝背 + 青蓝中段 + 银白腹 */
const SKIN_DEEP = 0x1e3d52; // 背部深靛蓝（水彩重色）
const SKIN_MID = 0x41799a; // 体侧青蓝
const SKIN_CREST = 0x9fc4cf; // 背脊浅色水渍
const SKIN_PALE = 0xe3edf0; // 腹部银白
const SKIN_PLEAT = 0x9db8c2; // 喉腹褶槽色
const FIN_PALE = 0xd7e4e8; // 长鳍肢淡青白（座头鲸特征）
const FLUKE_DEEP = 0x2c4f63; // 尾鳍深色
/** 藤壶浅壳色 */
...
/**
 * 水彩鲸皮顶点色（参考水彩座头鲸，确定性无随机）：
 *  - 背腹分层：椭球局部 +Y 背部深靛蓝，-Y 腹部银白，smoothstep 过渡；
 *  - 大块水彩渍：三组低频正弦在背上叠出深浅斑（顶点稀疏 → 软边渐晕）；
 *  - 背脊浅斑：参考图背脊中央的浅色水渍带；
 *  - 喉腹褶（opts.pleats）：头/吻腹面按 z 条纹压暗，读出平行喉褶。
...
function paintWhaleSkin(geometry, { pleats = false } = {}) {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    // 大块水彩渍：正弦斑（波长约为体长的 1/5，软边渐晕）
    const mottle = THREE.MathUtils.clamp(
      0.5 +
        (Math.sin(x * 0.42 + z * 0.24 + 1.7) * 0.46 +
          Math.sin(x * 0.23 - y * 0.31 + 4.2) * 0.32 +
          Math.sin(z * 0.55 + x * 0.13 + 2.6) * 0.22) *
          0.5,
      0,
      1
    );
    c.copy(mid).lerp(deep, mottle);
    // 背侧亮斑：参考图背脊的大块浅色水渍（只染背侧，亮度对撞）
    const bloomLight = THREE.MathUtils.clamp(
      0.5 +
        (Math.sin(x * 0.36 + z * 0.2 + 1.7) * 0.5 +
          Math.sin(x * 0.17 - y * 0.26 + 4.2) * 0.5) *
          0.5,
      0,
      1
    );
    c.lerp(crest, bloomLight * 0.72 * THREE.MathUtils.clamp(tTop + 0.32, 0, 1));
    // 腹部银白（只留最底一圈，侧面保持蓝色水彩）
    const bellyMix = THREE.MathUtils.smoothstep(-tTop, 0.34, 0.86);
    c.lerp(pale, bellyMix);
    // 喉腹褶：腹面前半段按 z 平行条纹压暗（头/吻/躯干前段）
    if (bellyMix > 0.25 && x > -2) {
      const groove = Math.sin(z * 2.6 + x * 0.5);
      const fade = pleats ? 1 : THREE.MathUtils.clamp((x - 4) / 10, 0, 1); // 躯干上向尾侧渐隐
      if (groove > 0.1) {
        c.lerp(pleat, (groove - 0.1) * 0.8 * Math.max(0.35, bellyMix) * fade);
      }
    }
    colors[i * 3] = c.r;
    colors[i * 3 + 1] = c.g;
...
function whaleProfile(X) {
  ...
  return { sy: 1 - 0.4 * rear, sz: 1 - 0.3 * rear - 0.14 * fore };
}
...
export function buildEcoLeviathanIsland(opts = {}) {
  ...
  // 大块刻面让巨鲸读出「手工积木」感，而不是一颗光滑的光蛋。
  {
    ...
    // 背鳍（座头鲸的小钩状背鳍）：地壳板后方脊线上的一枚小后掠鳍
    {
      const dorsal = new THREE.Mesh(
        new THREE.ConeGeometry(1.05, 2.6, 4),
        toonMat(SKIN_DEEP, { flatShading: true })
      );
      dorsal.name = "leviathan-dorsal";
      dorsal.scale.set(1.7, 1, 0.26);
      dorsal.position.set(-14.6, 5.15, 0);
      dorsal.rotation.z = 0.5; // 后掠（朝尾侧倾）
      dorsal.castShadow = true;
      addOutline(dorsal, OUTLINE_W);
      group.add(dorsal);
    }
  ...
  group.add(tailRoot);
  {
    ...
    // 尾鳍深色（参考水彩座头鲸的深靛蓝尾叶）
    const flukeMat = toonMat(FLUKE_DEEP, { flatShading: true });
    const wing = (side) => {
      const tri = new THREE.Mesh(new THREE.ConeGeometry(4.3, 11.2, 4), flukeMat);
      tri.name = `leviathan-fluke-${side < 0 ? "L" : "R"}`;
      tri.scale.set(1.12, 1, 0.16);
      tri.position.set(-4.8, 0, side * 2.35);
