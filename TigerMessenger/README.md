...
> **当前方向：保留用户原作，停止新模型替换。** 见 [原作保真重构](docs/原作保真重构.md)。下方前期 Godot/新资产包是历史实验，不代表原作已迁移完成。

> **2026-09 工程更新**：新增「家书」六章救援主线、球面碰撞修正、静态花丛合批、Blender 资产包与 Godot 可玩工程。
> 双击 `Start-Web.command` 验收完整 Web 世界；双击 `Play-Godot.command` 体验原生版本。
> 先读 [优化与工程指南](docs/优化与工程指南.md)。Godot 当前是紧凑的救援关卡，尚未迁移全部 Web 场景与载具。
...
**水晶城巡逻飞行器**（`player/aircraftRide.js`）：按 `V` 进入 / 退出编队领头机**第一人称驾驶舱**（观景不驾驶，与航空艇 / 气泡艇互斥）。另有 5 架浅蓝**小型侦察飞行器**驻守母塔与子塔之间；靠近后按 `F` 驾驶，`WASD` 前后/转向，`Space` / `Ctrl` 升降。侦察队轮换赶赴叹息之门，自动集结击落黑灰色中型伴飞鸟。
...
- **太古高山圣城**（`world/odysseyCitadel.js` + `citadelTown.js` + `citadelRange.js`）：Townscaper 式**规则生成**的圣城——五层贴地台地（每层 25×25 格 × 5 层，ASCII 单元格地图 + 邻接规则自动生成城垛 / 拱窗 / 黄金穹顶 / 正门门廊 / 拱形水门）+ 断崖瀑布 + 护城河（与运河水系打通）+ 梯湖水帘 + 台地鸟群（每台 20 只：白天漩涡 / 夜栖屋顶 / 纸士兵惊飞）
  - **Townscaper 全模拟**（`citadelTown.js`）：**15 色调色板**（白/米白/沙黄/柠黄/橙/砖红/陶土/褐/深褐/蓝灰/石板灰/蓝/藏青/青/松绿，字符 `0-9A-E` + 正门 `G`，旧档 W/L/B/D 自动迁移）+ **每块明度微抖**（5 档哈希）+ **户概念**（竖柱同色一户、户种子定窗密度 0.5/0.7/1.0 与门面朝向）+ **屋顶形状分类**（孤立块→四坡尖顶（高柱出更高尖顶）/ 条带→人字坡 / L 形→转角教堂尖塔 / 十字形→中心教堂尖塔 / 2×2→晒台+矮尖塔 / 大平顶→花园或晒台）+ **悬空支撑支架**（右键删除中间层后上层悬浮不塌陷，自动长出四个八面体式支柱边框——Townscaper flying buildings）+ **围合平顶花园**（草地+低栅栏+树+屋顶鸟）+ **底层围合广场**（石板铺装+拼缝）+ **水面小船/灯笼**点缀；每块在编辑器里摆放即自动「长」出全套构件
  ...
  - **运河交汇古堡**（`world/canalSystem.js` `buildCanalJunctionBox` + `odysseyCitadel` 第二实例）：**运河堤岸方框 = 城堡地基**——在运河环线月亮湖↔白鲸海湖航段，用运河同款立壁/土埂围出矩形干坞（**空中金色方框**：四角光柱+顶部亮线高过城堡，任何角度可见「高亮构建区」+ 四角灯塔），**堤岸条带逐点贴球面曲率**（立壁/土埂/灯塔/空中光柱全部按各点径向竖立，不斜插地面），**方框内实心平台：顶面是绝对水平的干台面**（旋转对齐球面径向，顶面四角等高 spread=0，城堡基座正好咬合；底面深埋进球面，边缘/四角贴合球面不悬空），**金色光罩 + 十字亮线浮在台面上高亮构建区——点高亮区即弹搭建面板并自动切到古堡目标**；方框内是**空地基**（默认无城堡，玩家自己搭建）的 **12 层 Townscaper 高塔**（**无台地模式**：`skipOuterTerrain` 不建外围台地/断崖岩石，镇体基座直接落在平台顶面，trim 裁剪自动跳过；面板「重置为 SPEC」在运河实例=清空地基）（独立存档键 `tm.citadel.levels.canal-junction.v1`）；搭建面板顶部「目标」下拉切换高山圣城 ⇄ 运河交汇古堡
...
- **莫比斯原初湖沼**（`world/moebiusSwamp.js`）：Y 轴锁死分层（地面 40 / 水面 25 / 湖底 10），遮天大树顶棚，仅萤火虫与发光花蕊照明的深蓝绿平涂世界；珍珠瓷白鲸 / 焦黑土著人偶 / 荷叶小舟 / 莫比斯虎；**默认放在书店镇中心**（坑缘擦书店门，湖边书店；可在地图编辑器拖动/存档），进入切换湖沼 BGM
...
├── shot-harness.html     # 资产清单截图 Harness（开发用；A/B 能力已并入主系统）
...
└── src/
    ...
    ├── ui/               # 运行时截图/OskSta A-B 工作台、地图/城堡/故事板/触控面板
