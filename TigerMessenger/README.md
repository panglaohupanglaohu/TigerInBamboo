...
- **高山圣城 · Townscaper 搭建面板**（`ui/citadelEditorPanel.js` + `citadelSceneEdit.js`）：乘坐航空艇时左键点选圣城打开；2D 平面图点格编辑（5 台地 × 25×25 格 × 5 层，4 种材质）+ **3D 直编辑**（左键叠块 / 侧面改色 / 空地加块 / 右键删块，悬停幽灵块）；`Q`/`E` 换层、`H` 隐藏高层、`Ctrl+S` 保存、`Ctrl+Z`/`Ctrl+Y` 撤销重做；编辑即时预览 3D（存档 `tm.citadel.levels.v1` / `.terrain.v1` / `.terrainObjects.v1`）
