...
这是探望模式"拖动时间轴回放"的基础：拖动滑块到任意时刻，四层切片同时呈现。
...
3. **封存是仪式，不是删除**。快照只读化，原件保留探望。
4. **回放不是对话**。探望接口是回放 —— 页面必须披露"这是回放，不是本人"。
...
- **探望接口 = 回放而非对话**：封存后页面进入探望模式 —— 四层只读、时间轴可拖动回放，顶部横幅常驻披露："**这是回放，不是本人**"。
- 探望模式仅本地（v1 不做分享/公开策略）。

### 阶段三：传递（v1 仅定义协议，不实现）

- **传递 = 复制，原件保留探望**。记忆核心与机体解耦：记忆可以脱离原机体，被另一只生物继承。
- **意愿协议**：封存者可留下一份意愿（will），声明继承者、偏好迁移清单、未发送意图的交接策略、是否保留探望。
...
- 遗存：手动封存（仪式性快照只读化）+ 本地探望模式（只读 + 时间轴回放 + "这是回放，不是本人"披露）。
- 传递：仅 `WILL_SCHEMA` 与意愿草稿生成，不实现迁移。
...
3. 探望模式开放策略：从"仅本地"扩展到只读分享链接 / 展厅探望角。
4. 传递协议落地：意愿执行器 —— 偏好迁移确认 UI、意图交接确认流、继承者的记忆核心初始化。
